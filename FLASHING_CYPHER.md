...
## Troubleshooting After Flash

### Firmware and AxeOS versions do not match

Install both files from the same Cypher release:

1. Flash `esp-miner.bin`.
2. Reboot.
3. Flash `www.bin`.
4. Reboot again.

Do not mix `esp-miner.bin` from one release with `www.bin` from another release.

### High ping or fallback pool warning

Cypher Gamma Max does not control internet latency. If AxeOS shows a high response
time, such as `1000 ms` or more, or shows `Using fallback pool`, check:

- Primary pool host and port.
- Fallback pool host and port.
- Wi-Fi signal strength.
- Whether the selected pool is geographically far away.
- Whether the pool is overloaded or rejecting the connection.

If the device is using fallback, share statistics may reset and stale shares can
increase until the primary pool is reachable again.

### High ASIC error rate or stale shares

For public installs, keep `predictiveAutotune` disabled until the miner is stable.
Use stock/default frequency and voltage first, then tune slowly.
